...
func CreateUser(isDemo bool, host string, databaseName string, username string, password string) error {
	...
	if isDemo {
		err = db.Exec("CREATE USER IF NOT EXISTS 'u_demo'@'192.168.%' IDENTIFIED BY '" + settings.DBDP + "'").Error
		if err != nil {
			return err
		}

		err = db.Exec("GRANT EXECUTE, CREATE, ALTER, DROP, INDEX, LOCK TABLES, SELECT, INSERT, UPDATE, DELETE ON `" + databaseName + "`.* TO 'u_demo'@'192.168.%'").Error
		if err != nil {
			return err
		}

		err = db.Exec("CREATE USER IF NOT EXISTS 'u_demo'@'127.0.0.1' IDENTIFIED BY '" + settings.DBDP + "'").Error
		if err != nil {
			return err
		}

		err = db.Exec("GRANT EXECUTE, CREATE, ALTER, DROP, INDEX, LOCK TABLES, SELECT, INSERT, UPDATE, DELETE ON `" + databaseName + "`.* TO 'u_demo'@'127.0.0.1'").Error
		if err != nil {
			return err
		}
	} else {
...
func OpenDatabase(databaseName string, username string, password string, host string, location string) (*gorm.DB, error) {
	...
	if databaseName == "main" {
		host = settings.DBHostMainDefault
	}

	if databaseName == "client" || databaseName == "client_demo" {
		host = settings.DBHostClientDefault
	}

	if databaseName == "analytics" {
		host = settings.DBHostAnalytics
	}

	if strings.Contains(databaseName, "db_") {
		if len(host) == 0 {
...
//RevokeDemo revoke priiliges from demo user on selected DB
func RevokeDemo(databaseName string) error {
	var err error
	var db *gorm.DB
	host := settings.DBHostMainDefault
	...
	db.Exec("REVOKE ALL PRIVILEGES ON `" + databaseName + "`.* FROM 'u_demo'@'%'")
	db.Exec("REVOKE ALL PRIVILEGES ON `" + databaseName + "`.* FROM 'u_demo'@'localhost'")
	db.Exec("REVOKE ALL PRIVILEGES ON `" + databaseName + "`.* FROM 'u_demo'@'127.0.0.1'")
	db.Exec("REVOKE ALL PRIVILEGES ON `" + databaseName + "`.* FROM 'u_demo'@'192.168.%'")
